...
## View 17 — Godot 4 campaign map (cities, roads, resources, armies via JS bridge)

const BOX := Vector4(6.4, 36.2, 18.9, 47.4) # lon0, lat0, lon1, lat1
const CX := 12.48
const CY := 41.89
const SCALE := 33.0 # как вид 14
...
const MIN_ZOOM := 0.18
const MAX_ZOOM := 3.4
const BASE_DIST := 48.0 # как вид 14
...
func _load_dem() -> void:
	# Web/export: PNG часто лежит как CompressedTexture2D — читаем через get_image()
	var img: Image = null
	...
	if img == null:
		...
		if err != OK:
			status_label.text = "DEM не загрузился — упрощённый контур Италии"
			dem_ready = false
			return
	if img.is_compressed():
		var derr := img.decompress()
		if derr != OK:
			status_label.text = "DEM decompress fail"
			dem_ready = false
			return
	# downsample — полный 2048×1920 в GDScript на Web зависает
	if img.get_width() > 640 or img.get_height() > 600:
		img.resize(640, 600, Image.INTERPOLATE_BILINEAR)
	dem_w = img.get_width()
	dem_h = img.get_height()
	dem_data.resize(dem_w * dem_h)
	# SRTM packed as R=hi, G=lo (как italy-dem.js)
	for y in dem_h:
		for x in dem_w:
	...
	dem_ready = true
	status_label.text = "DEM %dx%d" % [dem_w, dem_h]


func meters(lon: float, lat: float) -> float:
	if not dem_ready or dem_w <= 0:
		# без DEM: грубый полуостров, чтобы не было «зелёного квадрата»
...
func world_h(lon: float, lat: float) -> float:
	...
	if m <= 8.0:
		return 0.04
	# tall Alps + readable passes
	return 0.28 + m / 2950.0 * 26.0
...
func _paint_land_albedo(tw: int, th: int) -> ImageTexture:
	...
	for y in th:
		...
		for x in tw:
			...
			if m <= 8.0:
				continue
			# лёгкое затенение по склону
			var m2 := alpine_pass_carve(lon + 0.05, lat, meters(lon + 0.05, lat))
			var shade := clampf(1.0 + (m - m2) * 0.0015, 0.72, 1.2)
...
func _build_world() -> void:
	...
	e.ambient_light_color = Color(0.5, 0.58, 0.62)
	...
	sun.rotation_degrees = Vector3(-45, 0, 0) # с юга ~45°
	add_child(sun)

	var hemi := OmniLight3D.new()
	hemi.light_energy = 0.28
	hemi.omni_range = 500.0
	hemi.position = Vector3(0, 90, 0)
	add_child(hemi)

	# sea plane
	var sea := MeshInstance3D.new()
	var sea_mesh := PlaneMesh.new()
	sea_mesh.size = Vector2(1200, 1200)
	sea.mesh = sea_mesh
	...
	land_albedo = _paint_land_albedo(640, 600)
	...
	_add_army()
	if dem_ready:
		status_label.text = "Вид 17 · Godot 4 · Италия · ландшафт"
	else:
		status_label.text = "Вид 17 · упрощённый контур (DEM offline)"
...
func _make_height_mesh() -> ArrayMesh:
	...
	for j in range(ROWS + 1):
		...
		for i in range(COLS + 1):
			...
			var m := alpine_pass_carve(lon, lat, meters(lon, lat))
			var y := 0.0 if m <= 8.0 else (0.28 + m / 2950.0 * 26.0)
	...
	for j in range(ROWS):
		for i in range(COLS):
			...
			# только суша (+берег) — форма Италии, не зелёный прямоугольник
			var n_land := 0
			if m00 > 8.0: n_land += 1
			if m10 > 8.0: n_land += 1
			if m01 > 8.0: n_land += 1
			...
			var v0 := float(j) / float(ROWS)
			var v1 := float(j + 1) / float(ROWS)
			...
			_add_tri(st, p00, p01, p10, m00, m01, m10, Vector2(u0, v0), Vector2(u0, v1), Vector2(u1, v0))
			_add_tri(st, p10, p01, p11, m10, m01, m11, Vector2(u1, v0), Vector2(u0, v1), Vector2(u1, v1))
	st.generate_normals()
	return st.commit()
...
(function(){
  if (window.__godotView17Bridge) return;
  window.__godotView17Bridge = true;
